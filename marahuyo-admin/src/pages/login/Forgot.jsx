    ...
    if (valid) {
      try {
        const res = await axios.get(
          `http://localhost:8800get_forgot_credentials/${valid.user_id}`
        );
        ...
        setTimeout(() => {URL(valid.user_id)}, 3000);
      } catch (err) {
        console.log(err);
      }
    } else {
        ...
        <div className="flex flex-row-reverse">
          ...
          <div className="lg:w-6/12 h-screen flex flex-col items-center justify-center bg-white">
            <img
              ...
              width="75%"
            />
            {alerts.success.value ? <SuccessAlert text={alerts.success.text}/> : null}
