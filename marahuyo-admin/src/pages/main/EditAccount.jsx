...
const EditAccount = () => {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `http://localhost:8800/get_specific_info/${user_id}`
        );
  ...
  const handleOnClick = async () => {
    try {
      await axios.put(
        `http://localhost:8800/update_user_info/${user_id}`,
        data
      );
      setAlert(true);
      setTimeout(() => {
        window.location.href = `/users/${acc_id}`;
      }, 3000);
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <div>
      ...
      <form action="" className="p-10 sm:ml-64">
